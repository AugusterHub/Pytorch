...
class InceptionA(torch.nn.Module):
    def __init__(self, in_channels):
        ...
        self.brnach_pool = torch.nn.Conv2d(in_channels, 24, kernel_size=1)
    ...
    def forward(self, x):
        ...
        branch_pool = self.brnach_pool(branch_pool)

        # 将各个分支的结果以channel的维度拼接起来
        outputs = [branch1x1, branch5x5, branch3x3, branch_pool]
        return torch.cat(outputs, dim=1)

class GoogleNetOne(torch.nn.Module):
    def __init__(self):
        super(GoogleNetOne, self).__init__()
